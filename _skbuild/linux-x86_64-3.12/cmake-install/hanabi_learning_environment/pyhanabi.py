...
class HanabiState(object):
  ...
  def set_life_tokens(self, tokens):
    lib.StateSetLifeTokens(self._state, tokens)

  def set_information_tokens(self, tokens):
    lib.StateSetInformationTokens(self._state, tokens)

  def set_fireworks(self, fireworks):
    c_fireworks = ffi.new("int[]", fireworks)
    lib.StateSetFireworks(self._state, c_fireworks, len(fireworks))

  def set_discard_pile(self, cards):
    c_cards = ffi.new("pyhanabi_card_t[]", len(cards))
    for i, card in enumerate(cards):
      c_cards[i].color = card.color()
      c_cards[i].rank = card.rank()
    lib.StateSetDiscardPile(self._state, c_cards, len(cards))

  def set_hand(self, player_index, cards):
    c_cards = ffi.new("pyhanabi_card_t[]", len(cards))
    for i, card in enumerate(cards):
      c_cards[i].color = card.color()
      c_cards[i].rank = card.rank()
    lib.StateSetHand(self._state, player_index, c_cards, len(cards))
  ...
  def set_deck(self, cards):
    ...
    for i, card in enumerate(cards):
      c_cards[i].color = card.color()
      c_cards[i].rank = card.rank()
    lib.StateSetDeck(self._state, c_cards, len(cards))

  def set_cur_player(self, player_index):
    lib.StateSetCurPlayer(self._state, player_index)

  def set_hand_card(self, player_index, card_index, card):
    c_card = ffi.new("pyhanabi_card_t*")
    c_card.color = card.color()
    c_card.rank = card.rank()
    lib.StateSetHandCard(self._state, player_index, card_index, c_card)
